...
ggplot(data = diamonds_samples) + 
  geom_col(mapping = aes(x = cut, y = price, fill = clarity)) # use fill to show stacked bars 
...
ggplot(data = diamonds_samples) +
  ...
  geom_smooth(mapping = aes(x = carat, y = cut), color = "blue") # this doesn't work
